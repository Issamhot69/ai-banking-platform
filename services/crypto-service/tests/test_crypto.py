...
class TestWallets:
    async def test_create_btc_wallet(self, client, auth_headers):
        await client.delete_wallet if hasattr(client, "delete_wallet") else None
...
class TestTransactions:
    async def test_buy_crypto(self, client, auth_headers):
        ...
        if wallet_res.status_code == 400:
            wallets = await client.get("/api/v1/crypto/wallets", headers=auth_headers)
            wallet_id = next(w["id"] for w in wallets.json() if w["currency"] == "BNB")
        else:
